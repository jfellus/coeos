...
void		reset_pointer_bend_script_in_computer_park(t_gennet *gen, t_gennet_computer *computer, t_gennet_script_list *scriptlist, GdkEventButton *event)
{
  ...
  {
     if ((cur_computer = find_widget_computer_by_script(gen, comlinklist_out->comlink->out)) == NULL)
	continue;
...
void		update_position_script_in_computer_park(t_gennet *gen, t_gennet_computer *computer, GdkEventMotion *event)
{
  ...
  for (script_list = computer->scriptlist; script_list != NULL; script_list = script_list->next)
    {
      if ((script_list->script->pos_x = (((int)event->x) - script_list->script->pointer_x)) < 0)
	script_list->script->pos_x = 0;
      if ((script_list->script->pos_y = (((int)event->y) - script_list->script->pointer_y)) < 0)
	script_list->script->pos_y = 0;      
...
void		refresh_script(t_gennet *data, t_gennet_script *script, int x, int y)
{
  ...
  if ((!script) || (script->in_computer == GENNET_OUT))
    return ;
  ...
  gtk_widget_queue_draw_area(data->gui->DrawingArea, 
			     x - 5, y - 5,
			     IMG_SCRIPT_WIDTH + (data->gui->font_width * MAX_LOGICAL_NAME) + 10,
			     height + 10
			     );
}
  ...
  for (scriptlist = computer->scriptlist; scriptlist != NULL; scriptlist = scriptlist->next, nr++)
    {
	  gdk_draw_drawable(GTK_LAYOUT(widget)->bin_window,
			    ...
			    scriptlist->script->pixmap,
			    0, 0,
			    scriptlist->script->pos_x, scriptlist->script->pos_y,
			    IMG_SCRIPT_WIDTH, IMG_SCRIPT_HEIGHT);

	  if (scriptlist->script->pango)
	    gdk_draw_layout(GTK_LAYOUT(widget)->bin_window,
			    widget->style->fg_gc[GTK_WIDGET_STATE(widget)],
			    scriptlist->script->pos_x,
			    scriptlist->script->pos_y + VSPACE_COMPUTER_SCRIPT + IMG_SCRIPT_HEIGHT,
			    scriptlist->script->pango);
  ...
  for (computer = gen->computers; computer != NULL; computer = computer->next)
      ...
      if (computer->pango)
	{
	  gdk_draw_drawable(GTK_LAYOUT(widget)->bin_window,
			    ...
			    IMG_COMPUTER_WIDTH, IMG_COMPUTER_HEIGHT);
	  ...
	  gdk_draw_layout(GTK_LAYOUT(widget)->bin_window,
			  widget->style->fg_gc[GTK_WIDGET_STATE(widget)],
			  computer->pos_x,
			  computer->pos_y + VSPACE_COMPUTER_SCRIPT + IMG_COMPUTER_HEIGHT,
			  computer->pango);

	  if (computer->scriptlist != NULL)	    
	    DrawingArea_expose_event_script_in_computer(widget, computer, gen);	    
	}
    }
}
      ...
      for(polyline = comlink->polyline_list.first; polyline != NULL; polyline = polyline->next)
	{
	  ...
	  if (polyline->next != NULL)
	      ...
	      if (polyline->x > 0)
		{
		  ...
		  cairo_line_to(cr, (double) polyline->x_e + 20 * cos(7 * M_PI / 12),
				(double) polyline->y_e + dy);
		  cairo_line_to(cr, (double) polyline->x_e + 20 * cos(5 * M_PI / 12),
				(double) polyline->y_e + dy);		
		}
	      else
		{
		  ...
		  if (polyline->x_b > polyline->x_e)
		    angle += M_PI;
		  cairo_line_to(cr, (double) polyline->x_e + 20 * cos(11 * M_PI / 12 + angle ),
				(double) polyline->y_e + 20 * sin(11 * M_PI / 12 + angle));
		  cairo_line_to(cr, (double) polyline->x_e + 20 * cos(13 * M_PI / 12 + angle),
				(double) polyline->y_e + 20 * sin(13 * M_PI / 12 + angle));
		}
      ...
      if (script->in_computer == GENNET_OUT)
	{
	  ...
	  if (script->pango)
	    gdk_draw_layout(GTK_LAYOUT(widget)->bin_window,
			    widget->style->fg_gc[GTK_WIDGET_STATE(widget)],
			    script->pos_x,
			    script->pos_y + VSPACE_COMPUTER_SCRIPT + IMG_SCRIPT_HEIGHT,
			    script->pango);
...
void		DrawingArea_tooltips_event_callback(GtkWidget *widget, gpointer data)
{
  ...
  if ((script_list = find_widget_script_in_computer(gen, gen->gui->mouse_x, gen->gui->mouse_y)) != NULL)
	...
	return;
      ...
      val = promnet_prom_script_get_logical_name(script->prom_script);
      if ((val_name = rindex(val, '/')) != NULL)
	sprintf(cur, "logical_name : %s\n", val_name + 1);
      else
	sprintf(cur, "logical_name : %s\n", val);
      cur += strlen(cur);
      if (val != NULL)
	free(val);
      ...
      val = promnet_prom_script_get_path_prom_binary(script->prom_script);
      if ((val_name = rindex(val, '/')) != NULL)
	sprintf(cur, "binary : %s\n", val_name + 1);
      else
	sprintf(cur, "binary : %s\n", val);
      cur += strlen(cur);
      if (val != NULL)
	free(val);
      ...
      val = promnet_prom_script_get_prom_args_line(script->prom_script);
      if ((val_name = rindex(val, '/')) != NULL)
	sprintf(cur, "arguments : %s\n", val_name + 1);
      else
	sprintf(cur, "arguments : %s\n", val);
      cur += strlen(cur);
      if (val != NULL)
	free(val);
      ...
      val = promnet_prom_script_get_path_file_script(script->prom_script);
      if ((val_name = rindex(val, '/')) != NULL)
	sprintf(cur, "script : %s\n", val_name + 1);
      else
	sprintf(cur, "script : %s\n", val);
      cur += strlen(cur);
      if (val != NULL)
	free(val);
      ...
      val = promnet_prom_script_get_path_file_draw(script->prom_script);
      if ((val_name = rindex(val, '/')) != NULL)
	sprintf(cur, "draw : %s\n", val_name + 1);
      else
	sprintf(cur, "draw : %s\n", val);
	...
	free(val);
      ...
      val = promnet_prom_script_get_path_file_res(script->prom_script);
      if ((val_name = rindex(val, '/')) != NULL)
	sprintf(cur, "res : %s\n", val_name + 1);
      else
	sprintf(cur, "res : %s\n", val);
	...
	free(val);
      ...
      val = promnet_prom_script_get_path_file_config(script->prom_script);
      if ((val_name = rindex(val, '/')) != NULL)
	sprintf(cur, "config : %s\n", val_name + 1);
      else
	sprintf(cur, "config : %s\n", val);
      cur += strlen(cur);
      if (val != NULL)
	free(val);
      ...
      val = promnet_prom_script_get_path_file_dev(script->prom_script);
      if ((val_name = rindex(val, '/')) != NULL)
	sprintf(cur, "dev : %s\n", val_name + 1);
      else
	sprintf(cur, "dev : %s\n", val);
	...
	free(val);
      ...
      val = promnet_prom_script_get_path_file_prt(script->prom_script);
      if ((val_name = rindex(val, '/')) != NULL)
	sprintf(cur, "prt : %s\n", val_name + 1);
      else
	sprintf(cur, "prt : %s\n", val);
	...
	free(val);
      ...
      val = promnet_prom_script_get_path_file_gcd(script->prom_script);
      if ((val_name = rindex(val, '/')) != NULL)
	sprintf(cur, "gcd : %s\n", val_name + 1);
      else
	sprintf(cur, "gcd : %s\n", val);
	...
	free(val);
      ...
      val = promnet_prom_script_get_computer(script->prom_script);
      sprintf(cur, "computer : %s\n", val);
      cur += strlen(cur);
      if (val != NULL)
	free(val);
      ...
      gdk_draw_layout(GTK_LAYOUT(widget)->bin_window,
		      widget->style->fg_gc[GTK_WIDGET_STATE(widget)],
		      gen->gui->mouse_x, gen->gui->mouse_y, pango);
    }
	...
	free(val);
      ...
      val = promnet_prom_script_get_logical_name(prom_script);
      sprintf(cur, "from : %s\n", val);
      cur += strlen(cur);
      if (val != NULL)
	free(val);
...
gboolean	DrawingArea_button_press_event_callback(GtkWidget *widget, GdkEventButton *event, gpointer data)
{
  ...
  if (event->button == 1)
    DrawingArea_button1_press_event(widget, event, data);
  ...
  if ((script = find_widget_held_script(gen, event->x, event->y)) != NULL)
    {
      ...
      if ((script->pos_x = (((int)event->x) - script->pointer_x)) < 0)
	script->pos_x = 0;
      if ((script->pos_y = (((int)event->y) - script->pointer_y)) < 0)
	script->pos_y = 0;
    }
...
void		init_gui_drawing_area(t_gennet *data, GtkWidget *hBox)
{
  ...
  gtk_widget_set_events(data->gui->DrawingArea,
			GDK_EXPOSURE_MASK
...
void		init_gui_drawing(t_gennet *data)
{
  ...
  metrics = pango_context_get_metrics(context,
				      GTK_WIDGET(data->gui->DrawingArea)->style->font_desc,
...
t_polyline		*find_widget_comlink_bend(t_gennet *data, int x, int y)
{
  ...
  {
     if ((result = find_bend(&(comlink->polyline_list), x, y)) != NULL)
	return result;
  }
  return NULL;
}
...
t_polyline		*find_widget_comlink_polyline(t_gennet *data, int x, int y)
{
  ...
  for (comlink = data->comlinks; comlink != NULL; comlink = comlink->next)
  {
     if ((result = find_polyline(&(comlink->polyline_list), x, y)) != NULL)
	return result;
  }
  return NULL;
}
...
t_gennet_comlink			*find_widget_comlink(t_gennet *data, int x, int y)
{
  ...
  for (comlink = data->comlinks; comlink != NULL; comlink = comlink->next)
    {
       if (find_polyline(&(comlink->polyline_list), x, y) != NULL)
	  return comlink;
    }
  return NULL;
}
...
t_polyline		*find_widget_held_comlink_bend(t_gennet *data, int x, int y)
{
  ...
  for (comlink = data->comlinks; comlink != NULL; comlink = comlink->next)
  {
     if ((result = find_held_bend(&(comlink->polyline_list))) != NULL)
	return result;
  }
  return NULL;
}
...
t_polyline	*find_widget_held_comlink_polyline_list(t_gennet *data, int x, int y)
{
  ...
  for (comlink = data->comlinks; comlink != NULL; comlink = comlink->next)
  {
     if ((result = find_held_polyline(&(comlink->polyline_list))) != NULL)
	return result;
  }
  return NULL;
}
...
t_gennet_script_list	*find_widget_script_in_computer(t_gennet *data, int x, int y)
{
  ...
  for (computer = data->computers; computer != NULL; computer = computer->next)
  {
	...
	if (script_list->script->in_computer == GENNET_IN_COMPUTER)
	{
	   ...
	   if (
	      (x > (script_list->script->pos_x - SCRIPT_CLICK_ZONE)) && (x < (script_list->script->pos_x + IMG_SCRIPT_WIDTH + SCRIPT_CLICK_ZONE))
	      && (y > (script_list->script->pos_y - SCRIPT_CLICK_ZONE)) && (y < (script_list->script->pos_y + IMG_SCRIPT_HEIGHT + SCRIPT_CLICK_ZONE))
	      )
	      return script_list;
	}
     }
...
t_gennet_script		*find_widget_script(t_gennet *data, int x, int y)
{
  ...
  for (script = data->scripts; script != NULL; script = script->next)
    {
      if (script->in_computer == GENNET_IN_COMPUTER)
	continue;

      if (
	  (x > (script->pos_x - SCRIPT_CLICK_ZONE)) && (x < (script->pos_x + IMG_SCRIPT_WIDTH + SCRIPT_CLICK_ZONE))
	  && (y > (script->pos_y - SCRIPT_CLICK_ZONE)) && (y < (script->pos_y + IMG_SCRIPT_HEIGHT + SCRIPT_CLICK_ZONE))
	  )
	  return script;
    }
  return NULL;
...
t_gennet_script		*find_widget_held_script(t_gennet *data, int x, int y)
{
  ...
  for (script = data->scripts; script != NULL; script = script->next)
    if ((script->hold == 1) && (script->in_computer == GENNET_OUT))
      return script;
  return NULL;
}
