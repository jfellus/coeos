...
gboolean scribble_configure_event(GtkWidget * widget, GdkEventConfigure * event, gpointer data)
{
	...
	gdk_draw_rectangle(((TxDonneesFenetre *) data)->pixmap, widget->style->white_gc, TRUE, 0, 0, widget->allocation.width, widget->allocation.height);
...
gboolean scribble_expose_event(GtkWidget * widget, GdkEventExpose * event, gpointer data)
{
	...
	gdk_draw_drawable(widget->window, widget->style->fg_gc[GTK_WIDGET_STATE(widget)], ((TxDonneesFenetre *) data)->pixmap, event->area.x, event->area.y, event->area.x, event->area.y, event->area.width, event->area.height);

	return FALSE;
}

gboolean scribble_motion_notify_event(GtkWidget * widget, GdkEventMotion * event, gpointer data)
{
	int x, y;
