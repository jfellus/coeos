...
void ecrit_reseau_ascii()
{
    ...
    for (i = 0; i < sc->nbre_neurone; i++)
    {
        ...
        else
        {
            ...
            if (trouver_groupe(sc->neurone[i].groupe)->type == No_Winner_Colonne
                || trouver_groupe(sc->neurone[i].groupe)->type == No_PTM)
                fprintf(f1, "%f\n", sc->neurone[i].d);

            fprintf(f1, "%f %f %f\n", sc->neurone[i].posx, sc->neurone[i].posy,
                    sc->neurone[i].posz);
            pt = (sc->neurone[i]).coeff;
  ...
  for (i = 0; i < liaison_nbre ; i++) /* (int)(liaison->nbre) */
    {
        do
        {
            val =  neurone_deb + (int) ((((float) ((neurone_fin - neurone_deb) / pas_entree)) *
                        ((float) rand())) / ((float) modulo));
        }
                                  ...
                                  type_groupe * gpe_entree)
{
    ...
    if (dvx > (float) (entree_taillex / 2))
        dvx = (float) (entree_taillex / 2);
    if (dvy > (float) (entree_tailley / 2))
    ...
    for (u = -(int) dvx - offset_x; u <= (int) dvx; u++)
        for (v = -(int) dvy - offset_y; v <= (int) dvy; v++)
        {
            p = -1;
            /*      printf("u = %d , v=%d \n",u,v); */
            if (x + u >= 0 && x + u < entree_taillex)
                p = x + u;
            else if (x + u < 0 && entree_taillex > 1)
                p = x + u + entree_taillex;
            else if (x + u >= entree_taillex && entree_taillex > 1)
                p = x + u - entree_taillex;

            if (v + y >= 0 && v + y < entree_tailley)
                p = p + (v + y) * entree_taillex;
            else if (v + y < 0 && entree_tailley > 1)
                p = p + (entree_tailley + v + y) * entree_taillex;
            else if (v + y >= entree_tailley && entree_tailley > 1)
                p = p + (v + y - entree_tailley) * entree_taillex;

            if (p >= 0 && p < nbre)
            {
                /* d=sqrt(((float)(u))*((float)(u))+((float)(v))*((float)(v))); */
                /* if(d<liaison->dv && ((float)rand()/(float)modulo)<=liaison->proba) */
                if (fabs(u) <= dvx && fabs(v) <= dvy
                    && ((float) rand() / (float) modulo) <= proba)
                {
                             ...
                             type_groupe * gpe_entree)
{
    ...
    while (nbre_liaisons <= 0 && proba > 0.)
        nbre_liaisons =
            choix_liaisons_voisinage_core(liaison, neurone_deb, neurone_fin,
                                          gpe, x, y, pas_entree, gpe_entree);
...
float alea_gauss(float moyenne, float ecart_type)
{
    ...
    Xt = (float) sqrt(-2.0 * ecart_type * ecart_type * log(r1)) * cos(2.0 *
                                                                      pi *
                                                                      r2) +
        moyenne;
    return (Xt);
...
void creer_liaison_1_vers_1_neurone(type_groupe * groupe, type_liaison * liaison,
				    int *no_neurone, int pas_entree, int no_voie,
				    int no_gpe_liaison, int s_deb, int s_fin,
				    int e_deb, int e_fin, int *pos1)
{
  type_coeff *pt, *pt1;
  int j;
  ...
  if (groupe->type == No_Ou)
    ...
    {
      if (liaison->type == No_l_1_1_non_modif)
	pt->type = no_voie + liaison->mode;
...
void creer_liaison_1_vers_voisinage_neurone(type_groupe * groupe, type_liaison * liaison,
					    type_groupe * groupe_entree, int j,
					    int x, int y, int pas_entree,
					    int no_voie, int no_gpe_liaison, int e_deb, int e_fin)
{
  type_coeff *pt, *pt1;
  int i;
...
void creer_liaisons_entre_groupe(int e_deb, int e_fin, int s_deb, int s_fin,
				 int gpe, int gpe_entree, type_liaison * liaison,
				 int no_voie, int pas, int no_gpe_liaison)
{
  int x, y, z;
  ...
  if (groupe->type == No_Winner_Colonne || groupe->type == No_PTM || groupe->type == No_Winner_Macro
      ...
      || groupe->type == No_Macro_Colonne || groupe->type == No_KO_Discret || groupe->type == No_KO_Continu || groupe->type == No_Selverston)
    decalage = no_voie / 2;
  else decalage = 0;
