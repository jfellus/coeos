...
void lecture(int recursive_load, TxDonneesFenetre *onglet_leto)
{
	...
	for (i = 0; i < sc->nbre_groupe; i++)
	{
		...
		if (onglet_leto == NULL) sc->nbre_neurone += read_one_group_leto(f1, groupe, NULL, (void **) NULL);
		else
		{
	...
	for (i = 0; i < nbre_liaison_provisoire; i++)
	{
		...
		else read_one_link(f1, liaison, NULL, (void **) &onglet_leto->hashtab);
...
int read_macro(char *base_base, char *nom, int px, int py, int relatif, int mode_inclusion, int *selected_plane, TxDonneesFenetre *onglet_leto)
{
	int i;
	...
	*selected_plane = (100 + sc->nbre_macro_lues * 7);
	...
	for (i = 0; i < nbre_groupe_macro; i++)
	{
		...
		if (onglet_leto == NULL) sc->nbre_neurone += read_one_group_leto(f1, groupe_local, base_nom_complet, (void **) NULL);
		else
		{
	...
	for (i = sc->nbre_liaison; i < sc->nbre_liaison + nbre_liaison_macro; i++)
	{
		...
		if (onglet_leto == NULL) read_one_link(f1, liaison, base_nom_complet, NULL);
		else read_one_link(f1, liaison, base_nom_complet, (void **) &onglet_leto->hashtab);
