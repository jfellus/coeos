...
GtkWidget *do_find_completion(GtkWidget *do_widget, gpointer data)
{
  ...
  if (!sc->fm_window)
  {
    sc->fm_window = gtk_dialog_new_with_buttons("GtkEntryCompletion", NULL, GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT, GTK_STOCK_CLOSE, GTK_RESPONSE_NONE, NULL);
    ...
    g_signal_connect(G_OBJECT(entry), "activate", G_CALLBACK(find_group_name), data);
  }
