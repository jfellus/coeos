...
static void Tools_SelectFolder_browse_clicked_callback(GtkWidget *widget, gpointer data)
{
  ...
  dialog = gtk_file_chooser_dialog_new("Select folder", NULL, GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER, GTK_STOCK_OK, GTK_RESPONSE_ACCEPT, GTK_STOCK_CANCEL, GTK_RESPONSE_REJECT, NULL);
...
static void Tools_OpenFile_browse_clicked_callback(GtkWidget *widget, gpointer data)
{
  ...
  dialog = gtk_file_chooser_dialog_new("Open file", NULL, GTK_FILE_CHOOSER_ACTION_OPEN, GTK_STOCK_OK, GTK_RESPONSE_ACCEPT, GTK_STOCK_CANCEL, GTK_RESPONSE_REJECT, NULL);
...
static void SaveFile_browse_clicked_callback(GtkWidget *widget, gpointer data)
{
  ...
  dialog = gtk_file_chooser_dialog_new("Save file", NULL, GTK_FILE_CHOOSER_ACTION_SAVE, GTK_STOCK_OK, GTK_RESPONSE_ACCEPT, GTK_STOCK_CANCEL, GTK_RESPONSE_REJECT, NULL);
  result = gtk_dialog_run(GTK_DIALOG(dialog));
...
GtkWidget *script_edit_argument(t_prom_script *prom_script, void *argument, int max_length)
{
  char *val;
  GtkWidget *entry;
  entry = gtk_entry_new_with_max_length(max_length - 1);
...
void Edit_Properties(t_gennet *gen, t_gennet_script *pscript)
{
  ...
  if (pscript != NULL) prom_script = pscript->prom_script;
  else prom_script = NULL;
  ...
  e_psymb = script_edit_argument(prom_script, prom_script->path_file_symb, PATH_MAX);
  OpenFile_control_browse(gen->gui->NewScriptDialog, e_psymb, "Path to .symb :");

  e_pscript = script_edit_argument(prom_script, prom_script->path_file_script, PATH_MAX);
  OpenFile_control_browse(gen->gui->NewScriptDialog, e_pscript, "Path to .script :");

  e_pdraw = script_edit_argument(prom_script, prom_script->path_file_draw, PATH_MAX);
  OpenFile_control_browse(gen->gui->NewScriptDialog, e_pdraw, "Path to .draw :");

  e_pres = script_edit_argument(prom_script, prom_script->path_file_res, PATH_MAX);
  OpenFile_control_browse(gen->gui->NewScriptDialog, e_pres, "Path to .res :");

  e_pconfig = script_edit_argument(prom_script, prom_script->path_file_config, PATH_MAX);
  OpenFile_control_browse(gen->gui->NewScriptDialog, e_pconfig, "Path to .config :");

  e_pvar = script_edit_argument(prom_script, prom_script->path_file_var, PATH_MAX);
  OpenFile_control_browse(gen->gui->NewScriptDialog, e_pvar, "Path to .var :");

  e_pdev = script_edit_argument(prom_script, prom_script->path_file_dev, PATH_MAX);
  OpenFile_control_browse(gen->gui->NewScriptDialog, e_pdev, "Path to .dev :");

  e_pprt = script_edit_argument(prom_script, prom_script->path_file_prt, PATH_MAX);
  OpenFile_control_browse(gen->gui->NewScriptDialog, e_pprt, "Path to .prt :");

  e_pgcd = script_edit_argument(prom_script, prom_script->path_file_gcd, PATH_MAX);
  ...
  e_pdeploypath = script_edit_argument(prom_script, prom_script->path_prom_deploy, PATH_MAX);
  ...
  case GTK_RESPONSE_ACCEPT:
  {
    ...
    if (pscript != NULL) script = pscript;
    else
    {
    ...
    promnet_prom_script_set_path_file_symb(script->prom_script, (char *) gtk_entry_get_text(GTK_ENTRY(e_psymb)));
...
gboolean NewComputer_selected_computer(GtkTreeSelection *selection, GtkTreeModel *model, GtkTreePath *path, gboolean path_currently_selected, gpointer data)
{
...
void Computer_Edit_Computer(t_gennet *gen, t_gennet_computer *pcomputer)
{
  ...
  case GTK_RESPONSE_ACCEPT:
  {
    ...
    computer = Tools_NewComputer_OK(gen, gtk_entry_get_text(GTK_ENTRY(gaddress)), gtk_entry_get_text(GTK_ENTRY(glogin)), gtk_entry_get_text(GTK_ENTRY(gpath)), gtk_entry_get_text(GTK_ENTRY(gcpu)), gtk_entry_get_text(GTK_ENTRY(ginfos)), pcomputer);
    ...
    gtk_list_store_set(gen->gui->list_computer, &iter, COL_COMPUTER_ADDRESS, gtk_entry_get_text(GTK_ENTRY(gaddress)), COL_COMPUTER_LOGIN, gtk_entry_get_text(GTK_ENTRY(glogin)), COL_COMPUTER_PATH, gtk_entry_get_text(GTK_ENTRY(gpath)), COL_COMPUTER_CPU, gtk_entry_get_text(GTK_ENTRY(gcpu)), COL_COMPUTER_INFORMATION, gtk_entry_get_text(GTK_ENTRY(ginfos)), COL_COMPUTER_DATA, computer, -1);
...
void Edit_Computer(t_gennet *gen, t_gennet_computer *pcomputer)
{
  ...
  gen->gui->NewComputerDialog = gtk_dialog_new_with_buttons("New computer", GTK_WINDOW(gen->gui->WinMain), GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT, GTK_STOCK_OK, GTK_RESPONSE_ACCEPT, GTK_STOCK_CANCEL, GTK_RESPONSE_REJECT, NULL);
  ...
  gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(gen->gui->tree_view), COL_COMPUTER_ADDRESS, "Address", renderer, "text", COL_COMPUTER_ADDRESS, NULL);

  gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(gen->gui->tree_view), COL_COMPUTER_LOGIN, "Login", renderer, "text", COL_COMPUTER_LOGIN, NULL);

  gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(gen->gui->tree_view), COL_COMPUTER_PATH, "Path", renderer, "text", COL_COMPUTER_PATH, NULL);

  gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(gen->gui->tree_view), COL_COMPUTER_CPU, "CPU", renderer, "text", COL_COMPUTER_CPU, NULL);

  gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(gen->gui->tree_view), COL_COMPUTER_INFORMATION, "Informations", renderer, "text", COL_COMPUTER_INFORMATION, NULL);
...
static void menuScriptDelete(gpointer data)
{
  ...
  {
    if ((erase_without_saving(script)) == 0) return;
  }
  if (script->sc != NULL) gtk_notebook_remove_page(GTK_NOTEBOOK(((t_gennet *) data)->gui->tabbedWin), script->sc->num_onglet);
...
static void menuScriptInComputerDelete(gpointer data)
{
  ...
  {
    if ((erase_without_saving(scriptlist->script)) == 0) return;
  }
  if (scriptlist->script->sc != NULL) gtk_notebook_remove_page(GTK_NOTEBOOK(((t_gennet *) data)->gui->tabbedWin), scriptlist->script->sc->num_onglet);
...
void init_gui_winmain(t_gennet *data)
{
  ...
  sprintf(filename, "%s/bin_leto_prom/resources/coeos_icon.png", getenv("HOME"));
  ...
  else gtk_window_set_icon(GTK_WINDOW(data->gui->WinMain), icon);
  ...
  gtk_window_set_default_size(GTK_WINDOW(data->gui->WinMain), gdk_screen_width() - 450, gdk_screen_height() - 450);
...
static void OpenNetwork(GtkWidget *widget, gpointer data)
{
  ...
  case GTK_RESPONSE_ACCEPT:
    ...
    loadNetwork(gen, (char *) gtk_entry_get_text(GTK_ENTRY(e_netfile)), (char *) gtk_entry_get_text(GTK_ENTRY(e_cptfile)));
    break;
  case GTK_RESPONSE_REJECT:
    break;
  default:
